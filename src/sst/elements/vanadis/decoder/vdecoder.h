...
#define VANADIS_DECODER_ELI_STATISTICS                                                                \
    { "uop_cache_hit", "Count number of times the instruction micro-op cache is hit", "hits", 1 },    \
        ...
        { "ifetch_outside_space", "Instruction fetches whose address does not fit address_mask and were therefore faulted rather than sent", "operations", 1 }, \
    {                                                                                                 \
        "uops_generated",                                                                             \
...
public:
    ...
    SST_ELI_DOCUMENT_PARAMS(
            //{ "decode_q_len", "Number of entries in the decoded, but pending issue queue" },
                            ...
                            { "address_mask", "How wide an address this core's address space is. A fetch that does not fit it is a fault, not a request", "0xFFFFFFFFFFFFFFFF" })
    ...
    VanadisDecoder(ComponentId_t id, Params& params, SST::Output* output) : SubComponent(id)
    {
        ...
        // AN INSTRUCTION FETCH THE ADDRESS SPACE CANNOT HOLD. The same
        // declaration the load/store queue is given, applied to the other end
        // of the core; see fetchFitsSpace() below.
        address_mask               = params.find<uint64_t>("address_mask", 0xFFFFFFFFFFFFFFFFULL);
        stat_ifetch_outside_space  = registerStatistic<uint64_t>("ifetch_outside_space", "1");
        ...
        fdip_cfg.address_mask       = address_mask;
    ...
    // AN INSTRUCTION FETCH THE ADDRESS SPACE CANNOT HOLD.
    //
    // `address_mask` is this core's declaration of how wide an address is, and
    // the load/store queue already refuses a load or a store that does not fit
    // it (vbasiclsq.h, addressFitsSpace). The front end is the other end of the
    // same core and needs the same rule, for the same reason and with more at
    // stake: a speculating core follows a branch it has predicted and may be
    // wrong about, and an indirect branch on such a path computes its target
    // from a register that belongs to another iteration or to no iteration at
    // all. The target is then an address nowhere near the program, the fetch of
    // it is an ordinary read to the instruction cache, and nothing below the
    // core can tell it from a real one -- the coherence fabric is handed an
    // address no memory holds and can only stop the simulation.
    //
    // A real machine takes an instruction page fault on such a fetch. Nothing
    // is read, an instruction that faults is delivered in its place, and what
    // happens next is decided where every other fault is decided: on a
    // mispredicted path the fault is squashed with the rest of the path and
    // costs nothing, and at the head of the reorder buffer it is an address the
    // program really tried to execute and the core stops on it, naming it.
    //
    // That is what fetchFitsSpace() gates and what the fault below stands for.
    bool fetchFitsSpace(const uint64_t addr, const uint64_t width) const
    {
        if ( 0 == width ) { return true; }

        const uint64_t last = addr + width - 1;

        // The fetch wraps the top of the 64-bit space; no address space holds
        // it whatever the mask is.
        if ( last < addr ) { return false; }

        return ((addr & address_mask) == addr) && ((last & address_mask) == last);
    }

    // Said once per decoder per run, because a path that fetches one of these
    // usually fetches thousands and they are all the same event. The count is
    // the durable record.
    void noteIFetchOutsideSpace(const uint64_t addr, const uint64_t width)
    {
        stat_ifetch_outside_space->addData(1);

        if ( ! said_ifetch_outside_space ) {
            said_ifetch_outside_space = true;
            output_->verbose(CALL_INFO, 0, 0,
                "note: an instruction fetch at 0x%" PRI_ADDR " (width %" PRIu64 ") lies outside the address"
                " space this core is configured for (address_mask 0x%" PRI_ADDR "). Nothing is read and a"
                " faulting instruction is delivered in its place; if it is on a speculated path the squash"
                " discards it, and if it reaches the head of the reorder buffer the core stops on it."
                " Further occurrences are counted in the ifetch_outside_space statistic and not printed.\n",
                addr, width, address_mask);
        }
    }
    ...
    void setInstructionPointerAfterMisspeculate(const uint64_t newIP)
    {
        ...
        // The path that could not be fetched has been left behind.
        ifetch_fault_pending = false;
...
protected:
    ...
    Statistic<uint64_t>* stat_ifetch_outside_space;

    uint64_t frontend_override_bubble;
    uint64_t frontend_resteer_until;
    uint64_t frontend_bubble_cycle = 0;

    // The declaration of how wide an address is on this core, and whether a
    // fetch that did not fit it has already been turned into a fault on the
    // path the front end is currently on.
    uint64_t address_mask              = 0xFFFFFFFFFFFFFFFFULL;
    bool     ifetch_fault_pending      = false;
    bool     said_ifetch_outside_space = false;
